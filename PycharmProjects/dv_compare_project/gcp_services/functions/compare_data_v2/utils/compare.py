...
class CompareDataframes:
    ...
    def _compare_dataframes(df1, df2, cols=None):
        ...
        def _check_version(at_least_version='1.1.0'):
            ...
            if version.parse(pd_version) < version.parse(at_least_version):
                raise RuntimeError("When try to use pandas `compare` function, should be at least version: {}".format(at_least_version))
        _check_version()
    ...
    def compare(self, df_json, df_bq, cols=None):
        ...
        if diff_list_json:
            ...
            print("Different index: {} for JSON DF".format('\t'.join([str(t) for t in diff_index_json])))
        ...
        if diff_list_bq:
            ...
            print("Different index: {} for JSON DF".format('\t'.join([str(t) for t in diff_index_bq])))
