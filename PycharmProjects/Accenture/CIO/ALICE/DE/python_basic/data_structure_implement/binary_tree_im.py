# -*- coding:utf-8 -*-
"""This is to just implement the binary tree, for the binary tree,
the logic is really simple, if nothing then just create a node(node with
three properties: value, left and right.), if insert, then compare with root node,
if the value less then node, then create a node on left side, otherwise on right side.
