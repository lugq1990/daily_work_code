...
if __name__ == '__main__':
    ...
    print('*' * 20)
    # in fact you could see that, we could even add more and more parameters in the function f
    # but cause I just want to sum with integer number, others type isn't allowed, here you see,
    get_return_function_bad = f(1, [2, 32])  # two parameters, one is 1 with int, two is [2, 32] as list
    # here you won't see with wrong type error! why? cause not get the function run, to run, you have to ()
    get_return_function_bad()