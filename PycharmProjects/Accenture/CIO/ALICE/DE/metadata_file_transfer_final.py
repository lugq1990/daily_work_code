...
(select if(docmmruri is null, -99999, docmmruri) as docmmruri, doccategory as doccategory, 
 ...
 from documents where upper(doclanguage) in ('ENGLISH','N/A') and doclocation is not null) doc1,
...
regexp_extract(doclocation, '([^/.]+)\.[^.]*$',1) as documentname 
from documents
where upper(doclanguage) in ('ENGLISH','N/A') and doclocation is not null
)doc
...
import os
path = '/sftp/cio.alice/metadata/metadata_new'
org_path = os.path.join(path, '20190510_back')
for f in table_list:
    file_name = f.split('.')[0]
    print('Now is %s'%(file_name))
    os.system('cat %s/%s.txt|wc -l' % (org_path, file_name))
    os.system('cat %s/%s.txt|wc -l'%(path, file_name))
...
file_list = [x for x in os.listdir(path) if x.endswith('.txt')]

file_list = ["Alice_ClientLeader.txt","Alice_ClientRegion.txt","Alice_ClientServiceGroup.txt",
             "Alice_Client.txt","Alice_ContractCountry.txt","Alice_ContractLeader.txt",
             "Alice_Contract.txt","Alice_Document.txt",
             "Alice_MasterClient.txt","Alice_ClientCountry.txt"]
for f in file_list:
    file_path = os.path.join(path, f)
    print('now is:', file_path)
    with open(file_path, 'r') as fo:
        data = fo.read().split('\n')
        k_list = [x.split(';')[0] for x in data]
        if '-99999' not in k_list:
            print('here not -99999:', f)
        if len(set(k_list)) != len(k_list):
            print('for file: %s with double rows'%(f))
    fo.close()
...
file_list = [x for x in os.listdir(path) if x.endswith('.txt')]
key_list = []
for f in ['Alice_Mapping_Contract.txt']:
    with open(os.path.join(path, f), 'r') as fo:
        data = fo.read().split('\n')
        for m in data:
            if len(m.split(';')) <= 2:
                continue
            key_list.append(m.split(';')[-2])
    if len(key_list) - len(set(key_list)) != 0:
        print(f)
        print('with same key!')
