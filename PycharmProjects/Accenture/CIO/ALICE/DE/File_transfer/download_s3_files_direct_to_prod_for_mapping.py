...
def init_sftp():
    ...
    while True:
        try:
            ...
            curr_step += 1
            if curr_step >= retry_times or ssh is not None:
                break
        except AuthenticationException as e:
            time.sleep(60)
            logger.warning('When auth to the SSH, with error: {0} with {1:02d} times'.format(e, curr_step))
            # if the we couldn't get the connection to SFTP, just stop the program
            pass

    if ssh is None:
        logger.error("Couldn't init the ssh client with %d times"%(retry_times))
        raise SSHException("Couldn't init, just stop!")
