...
def pluginClassifier(X_train, y_train, X_test):
    ...
    sigma_dict = dict()
    for label in y_unique:
        sigma_dict[label] = np.mean(
            np.dot((X_train[y_train == label] - mu_dict[label]), (X_train[y_train == label] - mu_dict[label]).T))
    ...
    for i in range(len(y_unique)):
        print('label %d' % (int(y_unique[i])))
        print(sigma_dict[y_unique[i]])
    print(mu_dict)
    ...
    for data in X_test:
        ...
        for la in y_unique:
            ...
            score = pi_dic[la] * (1 / np.sqrt(np.abs(sigma_dict[la]))) * np.exp(
                -.5 * np.dot(np.dot((data - mu_dict[la]).T, (1 / sigma_dict[la])), (data - mu_dict[la])))
            prob_x.append(score)
